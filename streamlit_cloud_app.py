...
def clinical_interaction_page():
    ...
    with tabs[2]:
        ...
        # Record new audio section
        st.write("### Record Dental Conversation")
        
        # Add option to test microphone or use simulated data
        col1, col2 = st.columns(2)
        with col1:
            if st.button("Test Microphone", key="test_mic_btn"):
                ...
                if mic_working and test_audio:
                    st.audio(test_audio)
                    st.success("Your microphone is working correctly!")
                else:
                    st.error("Microphone test failed. Consider using simulated data instead.")
        
        with col2:
            use_sim_mode = st.checkbox("Use Simulated Data", key="use_sim_data", 
                                      help="Use this if your microphone isn't working")
        
        if use_sim_mode:
            # Use the simulated audio mode
            sim_success = use_simulated_audio_mode(patient_id, conn, c)
            if sim_success:
                st.session_state.conversation_text = st.session_state.conversation_text
        else:
        ...
        # After recording, check if we have conversation text to analyze
