        ...
        if not existing_record or not existing_record[0] or ('recording_new' in st.session_state and st.session_state.recording_new):
            ...
            if (not st.session_state.recording and st.session_state.audio_data is not None and not st.session_state.transcription_complete) or \
               (st.session_state.uploaded_file is not None and not st.session_state.transcription_complete):
                ...
                # Default transcription text - will be used if transcription fails
                ...
                # Simulate audio processing with progress bar
                st.write("#### Processing Audio...")
                progress_bar = st.progress(0)
                for i in range(100):
                    time.sleep(0.05)
                ...
                # Flag to track if we have a valid audio file
                valid_audio_file = False
                ...
                if st.session_state.uploaded_file is not None:
                    try:
                        ...
                        valid_audio_file = True
                        st.success(f"Audio file saved successfully to {audio_file_path}")
                    except Exception as e:
                    ...
                    try:
                        ...
                        if os.path.exists(sample_mp3_path):
                            ...
                            valid_audio_file = True
                            st.success(f"Sample audio file used: {audio_file_path}")
                        else:
                            # Create a dummy WAV file for demonstration
                            st.info("Creating a sample audio file for demonstration purposes...")
                            
                            try:
                                # Change the extension to .wav which is simpler to create
                                audio_file_path = audio_file_path.replace('.mp3', '.wav').replace('.MP3', '.wav')
                                
                                # Create a simple WAV file (RIFF header + basic data)
                                with open(audio_file_path, 'wb') as f:
                                    # RIFF header
                                    f.write(b'RIFF')
                                    ...
                                    f.write(b'\x00\x00\x00\x00\x00\x00\x00\x00')  # Sample data (silence)
                                valid_audio_file = True
                                st.success(f"Created dummy audio file: {audio_file_path}")
                            except Exception as e:
                ...
                # Transcribe audio using OpenAI Whisper if we have a valid audio file
                transcription_text = default_transcription  # Start with the default
                
                if valid_audio_file and audio_file_path and os.path.exists(audio_file_path):
                    st.write("#### Transcribing with Whisper...")
                    ...
                    if st.session_state.openai_api_key:
                        try:
                            ...
                            with open(audio_file_path, 'rb') as audio_file:
                                ...
                                st.success("Audio transcribed successfully with OpenAI Whisper!")
                        except Exception as e:
                            st.error(f"Error transcribing audio with OpenAI: {str(e)}")
                            st.info("Using default transcription as fallback.")
                    else:
                        st.warning("OpenAI API key is not configured. Using simulated transcription instead.")
                ...
                try:
                    ...
                    try:
                        ...
                        st.error(f"Error saving AI analysis to database: {str(db_error)}")
                    ...
                    with col1:
                        st.download_button(
                            ...
                            file_name=f"Clinical_Report_{patient[0]}_{patient[1]}_{datetime.now().strftime('%Y%m%d')}.md",
                            mime="text/markdown",
                            key=f"download_text_clinical_{patient_id}"
                        )
                    
                    with col2:
                        if PDF_EXPORT_AVAILABLE:
                            if st.button("Export as PDF", key=f"export_pdf_clinical_{patient_id}"):
                                try:
                                    pdf_link = export_report_as_pdf(ai_report, f"{patient[0]} {patient[1]}")
                                    st.markdown(pdf_link, unsafe_allow_html=True)
                                except Exception as e:
