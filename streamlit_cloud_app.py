        ...
        if not existing_record or not existing_record[0] or ('recording_new' in st.session_state and st.session_state.recording_new):
            ...
            if not st.session_state.recording and st.session_state.audio_data is not None and not st.session_state.transcription_complete:
                ...
                try:
                    ...
                    audio_file_path = f"data/audio/recording_{patient_id}_{int(time.time())}.wav"
                    
                    # For demonstration, we'll use a simulated audio file
                    # In a real implementation, you would save the actual recorded audio
                    # st.session_state.audio_data would contain the actual audio bytes
                    
                    # For now, we'll check if we have a real OpenAI client to use Whisper API
                    if client:
                        try:
                            # For demonstration purposes, we'll simulate having an audio file
                            # In a real implementation, you would use the actual audio file
                            
                            # Simulate creating a test audio file if needed for demonstration
                            # This is just for testing - in production you'd use the real recorded audio
                            if not os.path.exists(audio_file_path):
                                # Create the audio directory if it doesn't exist
                                os.makedirs(os.path.dirname(audio_file_path), exist_ok=True)
                                
                                # This is just a placeholder - in a real app, you'd save the actual recorded audio
                            ...
                            with open(audio_file_path, 'rb') as audio_file:
                                # This would be a real API call with a real audio file
                                try:
                                    # Attempt to transcribe with Whisper API
                                    # Note: This will fail with our dummy file, which is expected
                                    # In a real implementation with a valid audio file, this would work
                                    ...
                                    # If Whisper API fails (expected with our dummy file), use sample transcription
                            ...
                            # Don't store the audio file path if we couldn't create it
                            audio_file_path = None
                    else:
                        # If no OpenAI client, use sample transcription
                        transcription_text = """
                        Doctor: Hello, how are you feeling today?
                        Patient: I've been having some pain in my lower right molar for about a week now.
                        Doctor: I see. Can you describe the pain? Is it constant or does it come and go?
                        ...
                        # Don't store the audio file path if we don't have a client
                        audio_file_path = None
                ...
                try:
                    ...
                    with col1:
                        st.download_button(
                            label="Download as Text",
                            data=ai_report,
                            file_name=f"Clinical_Report_{patient_name}_{datetime.now().strftime('%Y%m%d')}.md",
                            mime="text/markdown"
                        )
                    ...
                    with col2:
                        if PDF_EXPORT_AVAILABLE:
                            if st.button("Export as PDF"):
                                try:
                                    pdf_link = export_report_as_pdf(ai_report, patient_name)
                                    st.markdown(pdf_link, unsafe_allow_html=True)
                                except Exception as e:
                                    st.error(f"Error generating PDF: {e}")
                                    st.markdown(create_download_link(ai_report, 
                                                                   f"Clinical_Report_{patient_name}_{datetime.now().strftime('%Y%m%d')}.md", 
                                                                   "Download as Markdown instead"), 
                                              unsafe_allow_html=True)
                        else:
                            st.info("PDF export requires additional packages. Install pdfkit and markdown packages for PDF export functionality.")
