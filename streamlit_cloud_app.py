            ...
            if submitted:
                ...
                try:
                    ...
                    if existing:
                        ...
                        c.execute(
                            "UPDATE questionnaires SET questionnaire_data = ?, completion_date = CURRENT_TIMESTAMP WHERE id = ?",
                            (str(medical_data).replace("'", "''"), existing[0])
                        )
                    else:
                        # Insert new questionnaire
                        c.execute(
                            "INSERT INTO questionnaires (patient_id, questionnaire_type, questionnaire_data, completion_date) VALUES (?, ?, ?, CURRENT_TIMESTAMP)",
        ...
        if existing_analysis and existing_analysis[0]:
            ...
            with col2:
                if PDF_EXPORT_AVAILABLE:
                    if st.button("Export as PDF", key="export_existing_pdf"):
                        try:
                            pdf_link = export_report_as_pdf(existing_analysis[0], patient_name)
                            st.markdown(pdf_link, unsafe_allow_html=True)
                        except Exception as e:
                            st.error(f"Error generating PDF: {e}")
                            st.markdown(create_download_link(existing_analysis[0], 
