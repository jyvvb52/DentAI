    ...
    with tabs[0]:
        ...
        if not existing_record or not existing_record[0] or ('recording_new' in st.session_state and st.session_state.recording_new):
            ...
            if st.session_state.recording:
                ...
                # If we reach here, it means the progress bar completed without user stopping
                # We'll automatically stop the recording
                if st.session_state.recording:
                    st.session_state.recording = False
                    st.experimental_rerun()
            ...
            if not st.session_state.recording and st.session_state.audio_data is not None and not st.session_state.transcription_complete:
                ...
                try:
                    ...
                    c.execute("SELECT id FROM clinical_records WHERE patient_id = ? AND record_type = 'transcription'", (patient_id,))
                    existing_id = c.fetchone()
                    
                    if existing_id:
                        # Update existing record
                        c.execute("UPDATE clinical_records SET record_data = ?, timestamp = ? WHERE id = ?", 
                                 (transcription_text, datetime.now().strftime('%Y-%m-%d %H:%M:%S'), existing_id[0]))
                    else:
                        # Insert new record
                        c.execute("INSERT INTO clinical_records (patient_id, record_type, record_data, timestamp) VALUES (?, ?, ?, ?)",
                                 (patient_id, 'transcription', transcription_text, datetime.now().strftime('%Y-%m-%d %H:%M:%S')))
                    ...
                    # Check if a record already exists
                    c.execute("SELECT id FROM clinical_records WHERE patient_id = ? AND record_type = 'ai_analysis'", (patient_id,))
                    existing_id = c.fetchone()
                    
                    if existing_id:
                        # Update existing record
                        c.execute("UPDATE clinical_records SET record_data = ?, timestamp = ? WHERE id = ?", 
                                 (ai_report, datetime.now().strftime('%Y-%m-%d %H:%M:%S'), existing_id[0]))
                    else:
                        # Insert new record
                        c.execute("INSERT INTO clinical_records (patient_id, record_type, record_data, timestamp) VALUES (?, ?, ?, ?)",
                                 (patient_id, 'ai_analysis', ai_report, datetime.now().strftime('%Y-%m-%d %H:%M:%S')))
...
def generate_ai_analysis(data):
    ...
    try:
        ...
        # Get chief complaint
        c.execute("SELECT record_data FROM clinical_records WHERE patient_id = ? AND record_type = 'chief_complaint'", (patient_id,))
        chief_complaint_data = c.fetchone()
        chief_complaint = chief_complaint_data[0] if chief_complaint_data else "No chief complaint recorded"
        
        # Get examination data
        c.execute("SELECT record_data FROM clinical_records WHERE patient_id = ? AND record_type LIKE '%examination%'", (patient_id,))
