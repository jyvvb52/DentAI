    ...
    with tabs[6]:
        ...
        with col1:
            if not st.session_state.is_recording:
                if st.button("🎤 Start Audio Recording", key="start_recording", use_container_width=True):
        ...
        if st.session_state.is_recording:
            ...
            try:
                if record_audio_chunk(chunk_filename):
                    ...
                    chunk_transcription = transcribe_audio(chunk_filename)
                    if chunk_transcription:
                        st.session_state.conversation_text += " " + chunk_transcription
                        st.session_state.audio_chunks.append(chunk_filename)
                        transcription_placeholder.markdown(st.session_state.conversation_text)
                    else:
                        # If transcription fails, still keep the audio chunk
                        st.session_state.audio_chunks.append(chunk_filename)
                        transcription_placeholder.markdown(st.session_state.conversation_text + " [Recording in progress...]")
            except Exception as e:
                st.error(f"Error during recording or transcription: {str(e)}")
        ...
        # Analysis and save options for the current recording
            ...
            if not st.session_state.current_analysis:
                with st.spinner("Analyzing conversation..."):
                    st.session_state.current_analysis = generate_ai_analysis({
                        'patient_id': patient_id,
                        'patient_name': f"{patient[0]} {patient[1]}",
                        'transcription': st.session_state.conversation_text
                    })
