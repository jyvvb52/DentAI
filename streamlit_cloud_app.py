...
def init_db():
    ...
    if c.fetchone()[0] == 0:
        ...
        demo_user_id = c.fetchone()[0]
        
        demo_patients = [
            ("John Doe", "1980-05-15", "555-123-4567", "Regular check-up patient", demo_user_id),
            ("Jane Smith", "1992-08-23", "555-987-6543", "History of gum disease", demo_user_id),
            ("Robert Johnson", "1975-11-30", "555-456-7890", "Needs crown replacement", demo_user_id)
        ]
        for patient in demo_patients:
            c.execute('''
            INSERT INTO patients (name, date_of_birth, contact_info, medical_notes, dentist_id)
            VALUES (?, ?, ?, ?, ?)
            ''', patient)
...
def login(username, password):
    ...
    c.execute("SELECT id, username, full_name FROM users WHERE username = ? AND password = ?", (username, hashed_password))
    user = c.fetchone()
    ...
    if user:
        ...
        st.session_state.user_id = user[0]  # Set the user_id from the database
        st.session_state.user_full_name = user[2] if user[2] else username  # Set full name or username
...
def dashboard_page():
    ...
    st.header(f"Dashboard Overview - {current_date}")
    ...
    try:
        if 'user_id' not in st.session_state:
            st.session_state.user_id = 1  # Fallback to a default ID (the demo user)
            st.warning("User ID not found in session. Using default user.")
            
        c.execute("SELECT COUNT(*) FROM patients WHERE dentist_id = ?", (st.session_state.user_id,))
        total_patients = c.fetchone()[0]
        col1.metric("Total Patients", total_patients)
    except Exception as e:
        col1.metric("Total Patients", "Error")
        st.error(f"Error retrieving patient count: {str(e)}")
    
    # Today's Appointments (placeholder)
    col2.metric("Today's Appointments", "0")
    ...
    # AI Analyses
    try:
        c.execute("SELECT COUNT(*) FROM clinical_records WHERE ai_analysis IS NOT NULL")
        ai_analyses = c.fetchone()[0]
        col4.metric("AI Analyses", ai_analyses)
    except Exception as e:
        col4.metric("AI Analyses", "Error")
        st.error(f"Error retrieving AI analysis count: {str(e)}")
    ...
    try:
        if 'user_id' not in st.session_state:
            st.session_state.user_id = 1  # Fallback to a default ID
            
        c.execute("SELECT id, name, date_of_birth FROM patients WHERE dentist_id = ? ORDER BY created_at DESC LIMIT 5", (st.session_state.user_id,))
        patients = c.fetchall()
        ...
        if patients:
            ...
            # Create CSS for highlighting rows on hover
            st.markdown("""
            <style>
            .dataframe tr:hover {
            ...
            # Add button to go to Patient Management
            if st.button("View All Patients", key="dashboard_view_all_patients"):
                st.session_state.current_page = "patients"
                st.rerun()
            
        else:
            st.info("No patients found. Add patients in the Patient Management page.")
    except Exception as e:
    ...
    try:
        c.execute('''
        SELECT cr.id, p.name as patient_name, cr.record_date 
        ...
        if records:
            ...
            record_data = []
            for r in records:
                record_id, patient_name, record_date = r
                record_data.append({
                    "ID": record_id,
            ...
            # Add button to go to Clinical Interaction
            if st.button("New Clinical Interaction", key="dashboard_new_clinical"):
                st.session_state.current_page = "clinical"
                st.rerun()
        else:
            st.info("No clinical records found. Start a clinical interaction to create records.")
    except Exception as e:
...
def patients_page():
    ...
    # Tab 1: View and manage patients
    with tab1:
        ...
        try:
            # Fetch patients from database with search filter
            if search_query:
                c.execute("""
                SELECT id, name, date_of_birth, contact_info
                FROM patients 
                WHERE dentist_id = ? AND name LIKE ? 
                ORDER BY name
            ...
            if patients:
                ...
                # Display patients with interactive elements
                for i, row in df.iterrows():
                    ...
                    with col5:
                        if st.button(f"View Patient {row['ID']}", key=f"view_patient_{row['ID']}"):
                            st.session_state.selected_patient = row['ID']
                            st.session_state.clinical_interaction_patient = row['ID']
                            st.session_state.current_page = "clinical"
                            st.rerun()
                    
                    st.markdown("---")
            else:
                st.info("No patients found. Add a new patient using the 'Add New Patient' tab.")
        except Exception as e:
            st.error(f"Error retrieving patient data: {str(e)}")
    ...
    with tab2:
        ...
        with st.form("add_patient_form"):
            ...
            medical_notes = st.text_area("Medical History Notes", height=150)
            ...
            if submit_button:
                if full_name:
                    try:
                        ...
                        INSERT INTO patients (name, date_of_birth, contact_info, medical_notes, dentist_id)
                        VALUES (?, ?, ?, ?, ?)
                        """, (full_name, dob.isoformat(), contact_info, medical_notes, st.session_state.user_id))
